...
def _chi2_goodness_of_fit(
    ...
    total = sum(observed.values())  # type: ignore[union-attr]
    if total == 0:
        return 0.0, 1.0
    # Filter out categories with zero expected count to avoid divide-by-zero warning
    pairs = [
        (observed.get(cat, 0), expected_dist.get(cat, 0.0) * total)  # type: ignore[attr-defined]
        for cat in expected_dist
        if expected_dist.get(cat, 0.0) * total > 0
    ]
    if not pairs:
        return 0.0, 1.0
    observed_counts, expected_counts = zip(*pairs)
    stat, p_val = chisquare(list(observed_counts), f_exp=list(expected_counts))
