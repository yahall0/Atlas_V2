...
def _load_csv(path: str) -> tuple[list[str], list[str], list[str]]:
    """Return (texts, labels, districts) from a CSV with 'text' and 'category' columns."""
    texts, labels, districts = [], [], []
    with open(path, newline="", encoding="utf-8") as fh:
        ...
        for row in reader:
            if "text" in row and "category" in row:
                ...
                districts.append(row.get("district", ""))
    if not texts:
        raise ValueError(f"No valid rows found in {path}")
    return texts, labels, districts
...
def evaluate(
    ...
    *,
    ...
    texts, true_labels, districts = _load_csv(test_data_path)
    ...
    filtered_texts, y_true_int, filtered_districts = [], [], []
    for txt, lbl, dist in zip(texts, true_labels, districts):
        if lbl in str2int:
            filtered_texts.append(txt)
            y_true_int.append(str2int[lbl])
            filtered_districts.append(dist)
        else:
    ...
    # Always write predictions CSV alongside the JSON report
    _pred_path = predictions_output_path or (
        str(Path(output_path).parent / "predictions.csv") if output_path
        else str(Path(checkpoint_dir) / "artifacts" / "predictions.csv")
    )
    _pred_out = Path(_pred_path)
    _pred_out.parent.mkdir(parents=True, exist_ok=True)
    with _pred_out.open("w", newline="", encoding="utf-8") as fh:
        writer = csv.writer(fh)
        writer.writerow(["text", "category", "predicted", "district"])
        for txt, true_int, pred_int, dist in zip(
            filtered_texts, y_true_int, y_pred_int, filtered_districts
        ):
            writer.writerow([
                txt,
                id2label.get(true_int, str(true_int)),
                id2label.get(pred_int, str(pred_int)),
                dist,
            ])
    print(f"Predictions saved to {_pred_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--predictions_output",
        default=None,
        help="Path to write the predictions CSV (text, category, predicted, district). "
             "Defaults to <checkpoint>/artifacts/predictions.csv.",
    )
    parser.add_argument(
        "--threshold",
        type=float,
